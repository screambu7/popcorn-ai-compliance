...
const form = useForm({
    ...
    regimen_fiscal: props.client.regimen_fiscal,
    regimen_fiscal_descripcion: props.client.regimen_fiscal_descripcion || '',
    actividad_vulnerable: props.client.actividad_vulnerable,
    actividades_vulnerables: props.client.actividades_vulnerables || [],
    obligado_antilavado: props.client.obligado_antilavado,
    es_pep: props.client.es_pep,
    nivel_riesgo: props.client.nivel_riesgo,
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                <Link :href="route('clients.show', client.id)" class="text-gray-500 hover:text-gray-700">&larr; Volver</Link>
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Editar: {{ client.razon_social }}</h2>
            </div>
        </template>
                ...
                <form @submit.prevent="submit" class="space-y-6 rounded-lg bg-white p-6 shadow">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input v-model="form.regimen_fiscal" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500" />
                        </div>
                    </div>

                    <h3 class="text-lg font-medium text-gray-900 border-b pb-2">Compliance</h3>
                    ...
                    <div class="space-y-4">
                        <div class="flex flex-wrap items-center gap-6">
                            <label class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700">Actividad Vulnerable</span>
                            </label>
                            <label class="flex items-center gap-2">
                                <input v-model="form.es_pep" type="checkbox" class="rounded border-gray-300 text-orange-500 focus:ring-orange-500" />
                                <span class="text-sm text-gray-700">PEP</span>
                            </label>
                            <label class="flex items-center gap-2">
                                <input v-model="form.active" type="checkbox" class="rounded border-gray-300 text-orange-500 focus:ring-orange-500" />
                    ...
                    <div class="flex items-center justify-end gap-4 border-t pt-4">
                        <Link :href="route('clients.show', client.id)" class="text-sm text-gray-600 hover:text-gray-800">Cancelar</Link>
